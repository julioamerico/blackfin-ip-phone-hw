...
static void usage(int status)
{
	fprintf(status ? stderr : stdout,
		"Usage: ad73311_test [options] <filename>\n"
		"\n"
		"Options:\n"
		"  -d          Use TDM mode\n"
...
//	ctrl_regs[5] = AD_CONTROL | AD_WRITE | CTRL_REG_A | SLB | MODE_DATA;
	...
	if(!tdm){
		perror("MENSAGEM DE DEBUG 11111");
		if (write(sport_fd, (char *)ctrl_regs, 12) < 0)
/*		if (write(sport_path, (char *)ctrl_regs, 12) < 0)*/
...
/*
	char buf_teste[20];
	int nbytes_teste;
	strcpy(buf_teste, "This is a test\n");
	nbytes_teste = strlen(buf_teste);

	if(write(sport_fd, buf_teste ,nbytes_teste) < 0)
		errp("SPORT!!! FALHA NA ESCRITA!!!");
*/
